...
export const getPrestadoresByServicioAndEspecialidad = async (
  ...
  try {
    ...
    const data = await getPool().request().query`
        SELECT TOP 20 P.id, P.firstname, P.lastname, P.email, P.description, P.phone, P.service_id, P.comuna_id, P.speciality_id, AVG(R.Score) as average_review, COUNT(R.id) as total_reviews 
        FROM Prestador P
