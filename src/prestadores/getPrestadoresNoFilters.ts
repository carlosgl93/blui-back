...
export const getPrestadoresNoFilters = async (res: Response) => {
  try {
    ...
    const data = await getPool().request().query`
        SELECT TOP 20 P.id, P.firstname, P.email, P.phone, P.address, P.city, P.region, P.country, P.service_id, P.speciality_id, P.lastname, P.comuna_id, AVG(R.Score) as average_review, COUNT(R.id) as total_reviews 
        FROM Prestador P
